...
import { TwitterApiIoProvider } from '../social/twitter-api-io.provider';
import {
  computeTweetRelevance,
...
export class TelegramBot {
  ...
  private getTwitter(): TwitterApiIoProvider | null {
    try {
      return this.moduleRef.get(TwitterApiIoProvider, { strict: false });
    } catch {
      return null;
    }
  }
  ...
  /** Delegate to the shared TwitterAPI.io provider. Caching + rate budgeting live there. */
  private async searchTweetsTop(query: string, queryType: 'Top' | 'Latest' = 'Top'): Promise<TweetRich[]> {
    return (await this.getTwitter()?.searchTweets(query, queryType)) ?? [];
  }

  private async fetchTweetById(id: string): Promise<TweetRich | null> {
    return (await this.getTwitter()?.fetchTweetById(id)) ?? null;
  }

  private async fetchTwitterProfile(handle: string): Promise<{ bio: string; followers: number; lastTweets: string[] } | null> {
    const p = await this.getTwitter()?.fetchUserProfile(handle);
    if (!p) return null;
    return { bio: p.bio, followers: p.followers, lastTweets: p.lastTweets };
  }
...
// TweetRich, LoreBudget, parseTwitterUrl, normalizeTweet, NARRATIVE_STOPWORDS,
// CRYPTO_CONTEXT_RE, extractProjectKeywords, computeTweetRelevance,
// rankTweetsForLore now live in ../social/twitter-relevance.ts — they're
// imported at the top of this file. Don't redeclare them here.
